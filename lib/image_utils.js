...
function processImage(image, metadata, asset, target) {
  if (target.default) {
...
module.exports = {
  convertImage: function(asset, targets) {
    const destDir = path.dirname(path.join(hexo.public_dir, asset.path));

    hexo.log.trace('Converting %s to %s targets',
    ...
    return fs.mkdirs(destDir)
      ...
      .then((metadata) => {
        const ps = targets.map((target) => processImage(image, metadata, asset, target));
